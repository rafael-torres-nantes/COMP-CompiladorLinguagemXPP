...
Token* Scanner::nextToken()
{
    ...
    while (true)
    {
        ...
        case 2: // Retorna identificador ou palavra reservada
            // Verifica se é uma palavra reservada
            if (lexeme == "class")
                token = new Token(CLASS, lexeme);
            else if (lexeme == "extends")
                token = new Token(EXTENDS, lexeme);
            else if (lexeme == "int")
                token = new Token(INT, lexeme);
            else if (lexeme == "string")
                token = new Token(STRING, lexeme);
            else if (lexeme == "break")
                token = new Token(BREAK, lexeme);
            else if (lexeme == "print")
                token = new Token(PRINT, lexeme);
            else if (lexeme == "read")
                token = new Token(READ, lexeme);
            else if (lexeme == "return")
                token = new Token(RETURN, lexeme);
            else if (lexeme == "super")
                token = new Token(SUPER, lexeme);
            else if (lexeme == "if")
                token = new Token(IF, lexeme);
            else if (lexeme == "else")
                token = new Token(ELSE, lexeme);
            else if (lexeme == "for")
                token = new Token(FOR, lexeme);
            else if (lexeme == "new")
                token = new Token(NEW, lexeme);
            else if (lexeme == "constructor")
                token = new Token(CONSTRUCTOR, lexeme);
            else
                token = new Token(ID, lexeme); // Identificador normal
            return token;
